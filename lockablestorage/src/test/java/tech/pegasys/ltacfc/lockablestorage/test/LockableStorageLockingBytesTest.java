...
public class LockableStorageLockingBytesTest extends AbstractLockableStorageTest {
  ...
  public void singleBlockchainNotLocked() throws Exception {
    ...
    this.storageWrapper.setBytes(key, val).send();
  }
  ...
  public void crossBlockchainNotLocked() throws Exception {
    ...
    assert(this.storageWrapper.getBytes(key).send()[0] == val[0]);
  }
  ...
  public void crossBlockchainDifferentCall() throws Exception {
    ...
    this.storageWrapper.setBytes(key, val1).send();
    ...
    // This will fail as the contract is locked.
    try {
      this.storageWrapper.getBytes(key).send();
      throw new Exception("Unexpectedly, no revert thrown");
    ...
    this.storageWrapper.setBytes(key, val2).send();
    assert(this.storageWrapper.getBytes(key).send()[0] == val2[0]);
  }
  ...
  public void commit() throws Exception {
    ...
    // Set some values.
    this.storageWrapper.setBytes(key0, val0).send();
    this.storageWrapper.setBytes(key1, val1).send();
    ...
    assert(this.storageWrapper.getBytes(key0).send()[0] == val0[0]);
    assert(this.storageWrapper.getBytes(key1).send()[0] == val1[0]);
  }
  ...
  public void ignore() throws Exception {
    ...
    assert(this.storageWrapper.getBytes(key0).send().length == 0);
    assert(this.storageWrapper.getBytes(key1).send().length == 0);
  }
